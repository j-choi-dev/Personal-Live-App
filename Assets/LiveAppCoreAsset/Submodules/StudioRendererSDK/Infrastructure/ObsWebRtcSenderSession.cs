...
namespace StudioRendererSDK.Infrastructure
{
    ...
    public sealed class ObsWebRtcSenderSession : MonoBehaviour, IWebRtcSenderSessionDomain
    {
        ...
        [SerializeField] private int sdpOperationTimeoutSeconds = 10;
        ...
        public async UniTask<bool> StartVideoLinkAsync(string endpoint, string token)
        {
            ...
            try
            {
                ...
                ValidateConfiguration(endpoint, normalizedToken);
                string normalizedEndpoint = NormalizeEndpoint(endpoint);
                PublishStatus("WebRTC 세션 초기화 요청 중");
                await ResetSessionAsync(normalizedEndpoint, normalizedToken, cancellationToken);
                ...
                await PostOfferAsync(normalizedEndpoint, normalizedToken, localDescription.Value.sdp, cancellationToken);

                PublishStatus("PC Browser Source의 Answer 대기 중");

                VideoSessionDescriptionResponse answerResponse = await WaitForAnswerAsync(normalizedEndpoint, normalizedToken, cancellationToken);
        ...
        private async UniTask WaitOperationAsync(RTCSessionDescriptionAsyncOperation operation, CancellationToken cancellationToken)
        {
            double deadline = Time.realtimeSinceStartupAsDouble + sdpOperationTimeoutSeconds;
            ...
            while (!operation.IsDone)
            {
                ...
                if (Time.realtimeSinceStartupAsDouble >= deadline)
                {
                    throw new TimeoutException("WebRTC SDP Offer 생성 시간 초과");
                }
        ...
        private async UniTask WaitOperationAsync(RTCSetSessionDescriptionAsyncOperation operation, CancellationToken cancellationToken)
        {
            double deadline = Time.realtimeSinceStartupAsDouble + sdpOperationTimeoutSeconds;
            ...
            while (!operation.IsDone)
            {
                ...
                if (Time.realtimeSinceStartupAsDouble >= deadline)
                {
                    throw new TimeoutException("WebRTC SDP 적용 시간 초과");
                }
        ...
        private void ValidateConfiguration(string endpoint, string token)
        {
            ...
            if (string.IsNullOrWhiteSpace(token) || token.Length < AgentTokenLength)
            {
